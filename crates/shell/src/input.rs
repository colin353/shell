...
impl Input {
    pub fn new(prompt: String, placeholder: String) -> Self {
        Self {
            prompt,
            placeholder,
        }
...
impl tui::Component<InputState> for Input {
    fn render(
        &mut self,
        t: &mut tui::Terminal,
        state: &InputState,
...
impl tui::AppController<InputState> for Input {
    ...
    fn render(
        ...
        if let Some(prev) = prev_state {
            ...
            // Clear out any excess characters if value is shorter
            if prev.value.len() > state.value.len() {
                t.move_cursor_to(self.prompt.len() + 1 + state.value.len(), 1);
                for _ in state.value.len()..prev.value.len() {
                    t.print(" ");
                }
            } else if prev.value.is_empty() && self.placeholder.len() > state.value.len() {
        ...
        t.move_cursor_to(self.prompt.len() + 1, 1);
        ...
        if state.focused {
            t.set_focus(self.prompt.len() + 1 + state.cursor, 1);
        } else {
            t.unset_focus();
        }
    }
...
pub fn transition(state: &InputState, event: KeyboardEvent) -> Transition<InputState> {
    match event {
        ...
        KeyboardEvent::CtrlW => {
            let mut new_state = (*state).clone();
            let new_cursor = find_prev_termpos(&state.value, state.cursor);
